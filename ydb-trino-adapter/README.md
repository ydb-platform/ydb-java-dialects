...
По умолчанию Trino `date` создаётся как YDB `Date`. Параметр JDBC URL `forceSignedDatetimes=true` переключает новые столбцы на `Date32`; существующие столбцы обоих типов поддерживаются независимо от параметра.
Для MERGE требуется стандартная подготовка запросов YDB JDBC — режим `disablePrepareDataQuery=true` не поддерживается. Здесь проверена совместимость только `Date` и `Date32`; предикаты ограничены диапазоном YDB `Date32`.
