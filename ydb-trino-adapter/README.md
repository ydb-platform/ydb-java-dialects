...
По умолчанию INSERT сначала записывает строки во временную таблицу YDB, затем
переносит их в целевую row-oriented таблицу одним `INSERT ... SELECT`.
Ошибка записи отдельного пакета не оставляет частично вставленные строки в цели.
Для временных таблиц нужны права CREATE/DROP; они удаляются при завершении или
отмене операции. Настройка `insert.non-transactional-insert.enabled=true`
включает прямую запись, при которой ошибка может оставить ранее записанные пакеты.
