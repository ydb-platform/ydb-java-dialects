...
| YDB | Trino | JDBC |
|---|---|---|
| `String` / `Bytes` | `varbinary` | `getBytes` / `setBytes` |
| `Utf8` / `Text` | `varchar` | `getString` / `setString` |

`String`/`Bytes` сохраняют исходные байты, включая некорректный UTF-8, нулевые
байты и пустые значения. Для них поддержаны чтение, запись, фильтры и JOIN.
Бинарные литералы в Trino записываются как `X'0080FF'`.
В прямом YQL `'текст'u` задаёт `Utf8`, а `'текст'` — `String/Bytes`.
Адаптер связывает значения через параметры: `setString` задаёт `Utf8`,
`setBytes` — бинарный `String`; пользовательские значения не вставляются в SQL.

Ранее адаптер ошибочно показывал `String`/`Bytes` как `varchar`. Теперь тип
существующих таких колонок в Trino — `varbinary`; если байты намеренно хранят
UTF-8 текст, его можно явно декодировать через `from_utf8(column)`.
...
логические, текстовые (`Utf8`/`Text`) и бинарные (`String`/`Bytes`) ключи,
даты и временные метки, nullable-ключи,
