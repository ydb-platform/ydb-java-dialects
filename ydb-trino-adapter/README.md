...
По умолчанию Trino `date` создаётся как YDB `Date`; параметр JDBC URL `forceSignedDatetimes=true` переключает новые столбцы на `Date32`.
Для записи обоих типов требуется стандартная подготовка YDB JDBC (`disablePrepareDataQuery=true` не поддерживается); предикаты существующих `Date` с границей вне диапазона типа пока возвращают неверный результат, поэтому поддержка не завершена.
