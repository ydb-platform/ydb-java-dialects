...
/**
 * Signed temporal columns with [connectYdb] `enableSignedDatetimes = true` and
 * `forceSignedDatetimes=true` on the JDBC URL (set explicitly by the caller).
 */
class SignedTemporalTypesIT {
    ...
    fun setUp() {
        jdbcUrl = buildString {
            ...
            append(ydb.endpoint())
            append(ydb.database())
            append("?disablePrepareDataQuery=true&forceSignedDatetimes=true")
            ydb.authToken()?.let { append("&token=").append(it) }
        }

        db = connectYdb(url = jdbcUrl, enableSignedDatetimes = true)
    }
    ...
    fun tearDown() {
        ...
        runCatching {
            transaction(db) {
                SchemaUtils.drop(SignedTemporal)
            }
        }
    ...
    @Test
    fun `connectYdb wires signed dialect and jdbc url has forceSignedDatetimes`() {
        val dialect = db.dialect as YdbDialect
        assertTrue(dialect.enableSignedDatetimes)
        assertTrue(jdbcUrl.contains("forceSignedDatetimes=true"), jdbcUrl)
    }

    @Test
    fun `should round-trip signed temporal columns`() = ydbTransaction(db) {
    ...
    @Test
    fun `should emit Date32 Datetime64 Timestamp64 ddl`() = ydbTransaction(db) {
        SchemaUtils.create(SignedTemporal)
